...
import { Users, Bed, Maximize2, Eye } from 'lucide-react';
        ...
        <CardContent className="space-y-4">
          {rooms.length > 0 ? (
            rooms.map((room) => (
              <div
                key={room.id}
                className="flex flex-col md:flex-row gap-4 p-4 border rounded-lg hover:border-secondary/50 transition-colors cursor-pointer"
                onClick={() => openRoomDetail(room)}
              >
                {/* Room Image */}
                <div className="w-full md:w-48 h-32 shrink-0 rounded-lg overflow-hidden bg-muted relative group">
